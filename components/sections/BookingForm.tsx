            ...
            {isOpen && (
                ...
                >
                    <motion.div
                        ...
                        className={`relative w-full max-w-7xl min-h-screen md:min-h-0 rounded-none md:rounded-sm overflow-hidden transition-all duration-1000 ${activeStep >= 2 ? 'pearl-monolith border-none' : 'luxury-monolith border border-white/10'}`}
                    >
                        {/* CLOSE BUTTON - ARCHITECTURAL PRECISION */}
                        <button
                            onClick={onClose}
                            className={`absolute top-6 right-6 md:top-10 md:right-10 z-[210] p-4 group transition-all duration-700 ${activeStep >= 2 ? 'text-black/20 hover:text-black' : 'text-white/20 hover:text-white'}`}
                        >
                            <X className="w-6 h-6 md:w-8 md:h-8 transition-transform group-hover:rotate-90 duration-700" strokeWidth={1} />
